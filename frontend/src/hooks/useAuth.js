...
export function AuthProvider({ children }) {
  ...
  const login = async (credentials) => {
    ...
    try {
      ...
      // 這裡示範簡單模擬登入，請依需求改成真實 API 呼叫
      // const user = await yourRealLoginAPI(credentials)
  ...
  const register = async (userData) => {
    ...
    try {
      ...
      if (!isValidEmail(userData.email)) throw new Error('請輸入有效的電子郵件地址')
      if (!userData.password) throw new Error('請輸入密碼')
      if (userData.password.length < 6) throw new Error('密碼長度至少需要6個字元')

      // 呼叫 Django REST Framework 的註冊 API
      ...
      if (!response.ok) {
        ...
        // 簡單合併後端錯誤訊息成字串
        const errorMessage = errorData.detail || JSON.stringify(errorData) || '註冊失敗'
